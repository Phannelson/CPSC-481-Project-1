...
def apply_action(state, action):
    ...
    for x in range(len(action)):
        new_state.completed_units += action[x].units
        new_state.courses_taken.append(action[x].name)
...
def astar_search(initial_state, available_courses):
    ...
    while open_list:
        ...
        temp = actions(current_state, available_courses)
        if temp == 1:
                return None
        else:
            for x in temp:
...
initial_state = State(0, ["CPSC 120"], 0, 0) 
